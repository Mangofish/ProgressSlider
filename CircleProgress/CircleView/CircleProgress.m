...
- (void)initialization {
    ...
    _textFont = [UIFont systemFontOfSize:0.15*CircleSelfWidth];
    ...
    _reduceAngle = CircleDegreeToRadian(0);
...
- (void)drawGradientInContext:(CGContextRef)ctx startingAngle:(float)startAngle endAngle:(float)endAngle intRadius:(CGFloat)intRadius outRadius:(CGFloat)outRadius withSubdiv:(int)subdivCount withCenter:(CGPoint)center {
    ...
    float angleDelta = (endAngle - 0)/subdivCount;//每一块的角度
    CGPoint p0,p1,p2,p3;
...
//          控制起始位置是否圆角
            if (i == 0 && _showPoint) {
                ...
                CGPathAddArc(halfCircle, &concat, roundCenter.x, roundCenter.y, (outRadius-intRadius)/2.0, M_PI, 2*M_PI, 1);
        ...
        if (_showProgressText) {
            ...
            //垂直居中
            CGRect r = CGRectMake((int)((CircleSelfWidth-stringSize.width)/2.0), (int)((CircleSelfHeight - stringSize.height)/2.0),(int)stringSize.width, (int)stringSize.height);
